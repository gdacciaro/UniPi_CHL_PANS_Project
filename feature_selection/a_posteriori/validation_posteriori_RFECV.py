...
def estimator_selection(X, y, k):
    ...
    estimators.append(SVC(random_state=42))
    estimators.append(LogisticRegression(random_state=42))
...
def RFECV_feature_selection2(training_data, validation_data, test_data, scaler_title, type,index):
    ...
    import pandas as pd
    data_meta = pd.read_csv(ROOT + '/data/a_posteriori/PANS_metabolomics.csv', sep=',')
    data_prote = pd.read_csv(ROOT + '/data/a_posteriori/PANS_proteomics.csv', sep=',')

    if type == "meta":
        meta_features = data_meta.iloc[:, indexes[0]]
        cols_name = meta_features.columns
    else:
        prote_features = data_prote.iloc[:, indexes[0]]
        cols_name = prote_features.columns

    write_csv_validation(X_train_transformed, X_val_transformed, X_test_transformed, y_train, y_val, y_test, id_train,
                         id_val, id_test, "rfe", scaler_title, tag=type, column_names=cols_name, index=index)
        ...
        for index, ((id_train,X_train, y_train), (id_val,X_val, y_val), (id_test,X_test, y_test)) in enumerate(db.get_data2(type)):
            ...
            for i in range(len(all_possible_scaled_X_train)):
                ...
                RFECV_feature_selection2((id_train, train_set, y_train), (id_val, val_set, y_val),
                                             (id_test, test_set, y_test), scaler_title, type,index)
